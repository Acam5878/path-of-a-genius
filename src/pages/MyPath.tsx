...
import { Plus, BookOpen, Target, Award, Crown, ArrowUpDown } from 'lucide-react';
import { AppLayout } from '@/components/layout/AppLayout';
import { Header } from '@/components/layout/Header';
import { SubjectCard } from '@/components/cards/SubjectCard';
import { Section } from '@/components/ui/section';
import { Button } from '@/components/ui/button';
import { subjects } from '@/data/geniuses';

// Simulated user data
const userSubjects = subjects.slice(0, 5).map((s, i) => ({
  ...s,
  progress: [75, 45, 30, 10, 0][i],
  status: i < 3 ? 'in_progress' : 'not_started',
}));

const completedSubjects = subjects.slice(5, 7).map(s => ({
  ...s,
  progress: 100,
  status: 'completed',
...
const MyPath = () => {
  const [sortBy, setSortBy] = useState<'progress' | 'age' | 'recent'>('progress');

  const totalSubjects = userSubjects.length + completedSubjects.length;
  const inProgress = userSubjects.filter(s => s.status === 'in_progress').length;
  const avgProgress = Math.round(userSubjects.reduce((acc, s) => acc + s.progress, 0) / userSubjects.length);
      ...
      <Header 
        ...
        rightActions={
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="icon" className="text-secondary hover:text-secondary">
      ...
      <div className="py-4 space-y-6">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center">
              ...
              <div className="font-mono text-xl font-bold text-foreground">{totalSubjects}</div>
            ...
            <div className="text-center">
              ...
              <div className="font-mono text-xl font-bold text-foreground">{inProgress}</div>
        ...
        <Section title="Currently Studying">
          <div className="px-4 space-y-3">
            {userSubjects.filter(s => s.status === 'in_progress').map((subject, i) => (
              <motion.div
                key={subject.id}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-sm">5/5 subjects tracked</p>
        ...
        <Section title="Not Started">
          <div className="px-4 space-y-3">
            {userSubjects.filter(s => s.status === 'not_started').map((subject, i) => (
              <motion.div
                key={subject.id}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              {completedSubjects.map((subject, i) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">Completed Jan 20, 2025</p>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        {/* Recommended */}
        <Section title="Recommended Next" action={{ label: 'Browse All', href: '/geniuses' }}>
          <div className="px-4 space-y-3">
            {subjects.slice(7, 9).map((subject, i) => (
              <motion.div
                key={subject.id}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
