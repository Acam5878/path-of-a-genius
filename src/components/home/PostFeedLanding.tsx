...
const DiagnosticResultsHero = ({ onSignIn, loading }: { onSignIn: (p: 'apple' | 'google') => void; loading: 'apple' | 'google' | null }) => {
  ...
  return (
    <section className="relative px-6 pt-16 pb-10 text-center overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[500px] h-[400px] bg-secondary/8 rounded-full blur-[120px] pointer-events-none" />
      ...
      >
        <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-secondary/60 mb-4">Your Brain Analysis</p>
        ...
        >
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <p className="text-[10px] font-mono uppercase tracking-wider text-white/30">Full Analysis Locked</p>
          </div>
          <div className="flex items-center justify-center gap-3">
            <div>
              <p className="text-white/40 text-xs">Current</p>
              <p className="font-mono font-bold text-xl text-white/60">{estimatedIQ}</p>
            </div>
            <ArrowRight className="w-4 h-4 text-secondary/50" />
            <div>
              <p className="text-secondary/60 text-xs">2-Week Projected</p>
              <p className="font-mono font-bold text-xl text-secondary">{projectedIQ}</p>
            </div>
          </div>
          <p className="text-[10px] text-white/25 mt-2">Sign up to unlock your personalised training plan</p>
        </motion.div>
...
export const PostFeedLanding = () => {
  ...
  const hasCompletedDiagnostic = localStorage.getItem('genius-academy-diagnostic-complete') === 'true';
